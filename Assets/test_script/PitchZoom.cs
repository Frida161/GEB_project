...
public class PitchZoom : MonoBehaviour
{
    ...
    public float initial_fieldofview =  99;
    public float magnified_fieldofview = 33;
    public float max_x_of_initial_fieldofview;
    public float min_x_of_initial_fieldofview;
    public float max_x_of_magnified_fieldofview;
    public float min_x_of_magnified_fieldofview;
    public float max_y_of_initial_fieldofview;
    public float min_y_of_initial_fieldofview;
    public float max_y_of_magnified_fieldofview;
    public float min_y_of_magnified_fieldofview;
    public float constant_z_position = -100f;
    ...
    private Vector3 cam_orginal_position;
    private float cam_original_orthographicSize;
    //public GameObject map;
    public static bool do_flag = true;

    void Start()
    {
        //相机的初始位置
        cam.transform.position = new Vector3(0f, 0f, -100);
        // map.transform.position = new Vector3(0f, 0f, 0);
        //一开始相机的fieldofview，手动测一下合适的视野范围
        cam.fieldOfView = initial_fieldofview;
        cam_orginal_position = cam.transform.position;
        cam_original_orthographicSize = cam.orthographicSize;
    ...
    public void back_to_offline(){
        cam.transform.position = cam_orginal_position;
        cam.fieldOfView = initial_fieldofview;
    }
    ...
    void UpdateWithTouch()
    {
        int touchCount = Input.touches.Length;
            ...
            if (touchCount == 1)
            {
                ...
                //根据field of view调整可移动的范围, 三维空间的直线函数，z为field of view， xy为范围;
                float boundMinX = (float)((cam.fieldOfView - initial_fieldofview)/(magnified_fieldofview-initial_fieldofview) * (min_x_of_magnified_fieldofview-min_x_of_initial_fieldofview)+ min_x_of_initial_fieldofview);
                float boundMaxX = (float)((cam.fieldOfView - initial_fieldofview) / (magnified_fieldofview - initial_fieldofview) * (max_x_of_magnified_fieldofview - max_x_of_initial_fieldofview) + max_x_of_initial_fieldofview);
                float boundMinY = (float)((cam.fieldOfView - initial_fieldofview) / (magnified_fieldofview - initial_fieldofview) * (min_y_of_magnified_fieldofview-min_y_of_initial_fieldofview)+min_y_of_initial_fieldofview);
                float boundMaxY = (float)((cam.fieldOfView - initial_fieldofview) / (magnified_fieldofview - initial_fieldofview) * (max_y_of_magnified_fieldofview-max_y_of_initial_fieldofview) + max_y_of_initial_fieldofview);

    cam.transform.position = new Vector3(
                    Mathf.Clamp(cam.transform.position.x, boundMinX, boundMaxX),
                     Mathf.Clamp(cam.transform.position.y, boundMinY, boundMaxY),
                      Mathf.Clamp(cam.transform.position.z, constant_z_position, constant_z_position)
                    );
    ...
    void OnPinch(Vector2 center, float oldDistance, float newDistance, Vector2 touchDelta)
    {
        ...
        if (controlCamera && cameraControlEnabled)
        {
            ...
            {
                cam.fieldOfView = Mathf.Clamp(cam.fieldOfView * oldDistance / newDistance, magnified_fieldofview, initial_fieldofview);
            }
        }
    }
